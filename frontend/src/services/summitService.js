  ...
  if (stored) {
    try {
      ...
      if (Array.isArray(parsed) && parsed.length >= INITIAL_SUMMITS.length) {
        ...
        const missing = INITIAL_SUMMITS.filter(
          (init) => !existingIds.has(Number(init.id)),
        );
...
export const isSummitActive = (summit) => {
  ...
  const eventDate =
    parseDateStr(summit.endDate) ||
    parseDateStr(summit.date) ||
    parseDateStr(summit.startDate);
