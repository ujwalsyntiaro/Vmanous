...
const ManageStudents = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to remove this student from the roster?")) {
  ...
  const collegeList = Array.from(new Set(students.map(s => s.collegeName).filter(Boolean)));
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            View confirmed paid registrations, digital passes, and attendance status by college.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          >
            <option value="All">All Colleges ({students.length} Enrolled)</option>
            {collegeList.map((colName) => {
              const count = students.filter(s => s.collegeName === colName).length;
              return (
                ...
                filteredStudents.map((stu, index) => (
                  <tr key={stu.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <p className="text-[11px] text-slate-500">{stu.email} &bull; {stu.phone}</p>
                    </td>

                    {/* College */}
                    <td className="py-3.5 px-4">
                      <p className="font-semibold text-slate-800 line-clamp-1">{stu.collegeName}</p>
                      <p className="text-[11px] text-slate-500">{stu.venueLocation || 'Main Campus'}</p>
                    </td>

                    {/* Program */}
                    <td className="py-3.5 px-4">
                      <p className="font-medium text-slate-800">{stu.programTitle}</p>
                      <p className="text-[11px] text-slate-500">{stu.degree} &bull; {stu.branch}</p>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => navigate('/pass', { state: { formData: { firstName: stu.studentName, email: stu.email, institution: stu.collegeName } } })}
