...
const ProgramCard = ({ summit, index = 0, isAdmin = false, onRegister, onEdit, onDelete }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-lg border border-slate-200/80 shadow-sm p-4 sm:p-5 flex flex-col relative overflow-hidden transition-all duration-300 hover:shadow-xl group h-full"
    >
      ...
      <div className="flex-1 relative z-10 flex flex-col">
        ...
        {/* Title */}
        <h3 className="text-lg md:text-xl font-bold text-slate-900 group-hover:text-emerald-700 transition-colors mb-2 tracking-tight">
          {summit.title}
        </h3>
        ...
        {/* Schedule & Timing Box */}
        <div className="p-2.5 rounded-md bg-slate-50 border border-slate-200/60 mb-3 space-y-1.5 shadow-2xs">
        ...
        {/* Subtitle / Objective */}
        {summit.subtitle && (
          <p className="text-xs font-medium text-slate-600 leading-snug mb-2.5">
            {summit.subtitle}
          </p>
        )}
        ...
        {summit.features && summit.features.length > 0 && (
          <div className="space-y-1.5 mb-3 pt-2 border-t border-slate-100 flex-1">
      ...
      {/* Card Footer Action */}
      <div className="mt-auto pt-2.5 border-t border-slate-100 relative z-10">
        {isAdmin ? (
